...
cat <(head -n -1 $template) ${WD}/7sk_custom_annots.txt <(python ${WD}/../create_annotations.py $nanocomp_res_kd $nanocomp_bed $template $tx 0.01 8 $real_start $rfam_id) <(echo //) > ${WD}/7sk_mettl3.sto
