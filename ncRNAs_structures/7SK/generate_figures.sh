...
nanocomp_bed="${BASEDIR}/nanocompore_pipelines/IVT_7SK/results/references/reference_transcriptome.bed"                                                                                                                                              
template="${WD}/7sk.sto" 
tx="ENST00000636484"
thr="0.05"
pval_col=8
...
cat <(head -n -1 $template) ${WD}/7sk_custom_annots.txt <(python ${WD}/../create_annotations.py $nanocomp_res_kd $nanocomp_bed $template $tx 0.01 8 $real_start $rfam_id ${WD}/kd_scale.svg) <(echo //) > ${WD}/7sk_mettl3.sto

cat <(head -n -1 $template) ${WD}/7sk_custom_annots.txt <(python ${WD}/../create_annotations.py $nanocomp_res_ivt $nanocomp_bed $template $tx 0.01 6 $real_start $rfam_id ${WD}/ivt_scale.svg) <(echo //) > ${WD}/7sk_ivt.sto

cat <(head -n -1 $template) ${WD}/7sk_custom_annots.txt <(python ${WD}/../create_annotations.py $nanocomp_res_dkc $nanocomp_bed $template $tx 0.01 8 $real_start $rfam_id ${WD}/dkc_scale.svg) <(echo //) > ${WD}/7sk_dkc1.sto
